...
class Campeonato:
    ...
    def jogar_rodada(self, mostrar_resultados=False):
        ...
        for equipe_casa, equipe_visitante in agendamento_jogos:
            ...
            resultados_rodada.append(jogo.resultado_final)
        ...
        self.exibir_classificacao_atual()  # Exibe a classificação atual
    ...
    def exibir_classificacao_atual(self):
        classificacao_atual = sorted(self.equipes, key=lambda equipe: equipe.pontos, reverse=True)
        print("Classificação atual:")
        for i, equipe in enumerate(classificacao_atual):
            print(f"{i+1}. {equipe.nome} - {equipe.pontos} pontos")
        print()
    ...
    def salvar_classificacao(self, arquivo):
        ...
        with open(arquivo, 'w', newline='') as file:
            ...
            writer.writerow(['Posição', 'Nome', 'Pontos', 'Rank'])
            for i, equipe in enumerate(classificacao):
                writer.writerow([i+1, equipe.nome, equipe.pontos, equipe.rank])
...
equipes = ler_equipes('portugal.csv')
